...
class CodeAnalysisController:
    ...
    def start_analysis(self, path: str, threshold: float, num_tests: int) -> Dict[str, Any]:
        """
        Analyzes a single Python file or an entire folder by performing AST-based static analysis,
        token-based clone detection, runtime analysis, and data flow analysis.

        Args:
            path (str): The path to a Python file or directory.
            threshold (float): The similarity threshold for analysis.
            num_tests (int): The number of test cases for runtime analysis.
        ...
        try:
            ...
            for file, tree in ast_trees.items():
                ...
                # Store data flow results using function names instead of filenames
                for func_name, func_data in data_flow_results.items():
                    all_data_flow_results[func_name] = func_data  # Now indexed by function names
            ...
            if all_similar_nodes or all_token_clones:
                ...
                valid_results = [res for res in combined_results if "function1_metrics" in res and "function2_metrics" in res]
